  ...
  while (Running) {
    ...
    while (SDL_PollEvent(&event)) {
      switch (event.type) {
        ...
        case SDL_KEYUP:
          renderer.OnKey(SDL_KEYUP, event.key.keysym.scancode);
          break;
        case SDL_KEYDOWN:
          renderer.OnKey(SDL_KEYDOWN, event.key.keysym.scancode);
          break;
...
void SDL2Wrapper::DrawPoint(int centreX, int centreY, int radius,
                            const RGB& rgb) {
