...
void Renderer2D::AddPoint(const Point2D& p) {
    Points.push_back(p);
}

void Renderer2D::AddVector(const Vector2D& v) {
    Vectors.push_back(v);
}
...
void Renderer2D::DrawPoints() {
    ...
    for (auto& point : Points) {
        Sdl->DrawPoint(
            point.X * GridStep + (OriginX * GridStep),
            point.Y * GridStep + (OriginY * GridStep),
            4, pointColor);
    }
}

void Renderer2D::DrawVectors() {
    vector<RGB> vectorColors = {
        RGB(33, 150, 243),
        RGB(76, 175, 80),
        RGB(255, 152, 0),
        RGB(156, 39, 176),
    ...
    for (auto& vect : Vectors) {
        Sdl->DrawLine(
            OriginX * GridStep, OriginY * GridStep,
            vect.X * GridStep + (OriginX * GridStep),
            vect.Y * GridStep + (OriginY * GridStep),
            vectorColors[c++]);
    }
}
...
void Renderer2D::DrawLines() {
    for (auto& line : Lines) {
        Sdl->DrawLine(
            line.P1.X * GridStep + (OriginX * GridStep),
