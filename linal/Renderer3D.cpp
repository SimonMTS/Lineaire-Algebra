...
void Renderer3D::Init() { Sdl->OnEvent(*this); }

void Renderer3D::AddLine(Line3D& l) { Lines.push_back(l); }

void Renderer3D::OnKey(int event, int key) {
  std::cout << "key press\n";
  if (key == SDL_SCANCODE_A) {
    Perspective = Perspective3D::Frontview;
    std::cout << "Front view\n";
  }
  if (key == SDL_SCANCODE_S) {
    Perspective = Perspective3D::Topview;
    std::cout << "Top view\n";
  }
  if (key == SDL_SCANCODE_D) {
    Perspective = Perspective3D::Sideview;
    std::cout << "Side view\n";
  }
  ...
  if (event == SDL_KEYUP && key == SDL_SCANCODE_KP_1) {
    Rotate(1);
    std::cout << "Rotate +x\n";
  }
  if (event == SDL_KEYUP && key == SDL_SCANCODE_KP_3) {
    Rotate(-1);
    std::cout << "Rotate -x\n";
  }

  if (event == SDL_KEYUP && key == SDL_SCANCODE_KP_4) {
    Rotate(2);
    std::cout << "Rotate +y\n";
  }
  if (event == SDL_KEYUP && key == SDL_SCANCODE_KP_6) {
    Rotate(-2);
    std::cout << "Rotate -y\n";
  }

  if (event == SDL_KEYUP && key == SDL_SCANCODE_KP_7) {
    Rotate(3);
    std::cout << "Rotate +z\n";
  }
  if (event == SDL_KEYUP && key == SDL_SCANCODE_KP_9) {
    Rotate(-3);
    std::cout << "Rotate -z\n";
  }

  if (event == SDL_KEYUP && key == SDL_SCANCODE_KP_0) {
    Rotate(1);
    Rotate(2);
    Rotate(3);
...
void Renderer3D::OnResize(int width, int height) {
  ...
  // OriginZ = (((Perspective == Perspective3D::Topview ? width : height) / 2) /
  // GridStep) / 2;
...
void Renderer3D::Rotate(const int dir) {
  ...
  Matrix3D tm2 = Matrix3D::GetTranslationMatrix(localOrigin.X, localOrigin.Y,
                                                localOrigin.Z);
  Matrix3D rm =
      Matrix3D::GetRotationMatrix(abs(dir), (dir > 0 ? 5.625 : -5.625));
